PLUGIN.name = "Modified Menu"
PLUGIN.author = "Fayz_Golden"
PLUGIN.desc = "Modifies character creation menu"
---------------------------------------------------
...
surface.CreateFont("nut_MenuButtonFont2", {
	...
	function PANEL:Init()
		...
		self.model:SetPos( self:GetWide()/13*1.7, self:GetTall()*line)
	...
	--	self.model:DockMargin(ScrW() * 0.175, ScrH() * 0.05, 0, ScrH() * 0.05)
		...
		if (nut.config.menuMusic) then
			...
			if (string.Right(lower, 4) == "http") then
